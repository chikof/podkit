...
const ATEXT_MAP: [bool; 128] = {
    let mut t = [false; 128];
    let mut b = b'a'; while b <= b'z' { t[b as usize] = true; b += 1; }
    b = b'A';         while b <= b'Z' { t[b as usize] = true; b += 1; }
    b = b'0';         while b <= b'9' { t[b as usize] = true; b += 1; }
    ...
    t[b'-' as usize] = true; t[b'/' as usize] = true;
    t[b'=' as usize] = true; t[b'?' as usize] = true;
    t[b'^' as usize] = true; t[b'_' as usize] = true;
    t[b'`' as usize] = true; t[b'{' as usize] = true;
    t[b'|' as usize] = true; t[b'}' as usize] = true;
    t[b'~' as usize] = true;
    t
...
impl std::fmt::Display for EmailError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DomainLabelEmpty          => write!(f, "domain contains an empty label (leading or consecutive dots)"),
            Self::DomainLabelTooLong        => write!(f, "domain label exceeds {MAX_LABEL_LEN} characters"),
            ...
            Self::DomainLabelInvalidChar(c) => write!(f, "invalid character in domain label: {c:?}"),
            Self::DomainMissingTld          => write!(f, "domain has no TLD (must contain at least one dot)"),
            ...
            Self::NonAscii(c)               => write!(f, "non-ASCII character {c:?}; encode international domains as punycode"),
        }
    }
}
...
impl<'a> ValidatedEmail<'a> {
    ...
    #[inline]
    pub fn normalize(self) -> String { normalize_email(self.0) }
}
...
fn validate_local(local: &str) -> Result<(), EmailError> {
    if local.is_empty()            { return Err(EmailError::LocalPartEmpty); }
    if local.len() > MAX_LOCAL_LEN { return Err(EmailError::LocalPartTooLong); }

    let bytes = local.as_bytes();
    if bytes[0] == b'.'               { return Err(EmailError::LocalPartLeadingDot); }
    if *bytes.last().unwrap() == b'.' { return Err(EmailError::LocalPartTrailingDot); }
    ...
    for (i, &b) in bytes.iter().enumerate() {
        ...
        if b == b'.' {
            if prev_dot { return Err(EmailError::LocalPartConsecutiveDots); }
            prev_dot = true;
...
fn validate_label(label: &str) -> Result<(), EmailError> {
    if label.is_empty()            { return Err(EmailError::DomainLabelEmpty); }
    if label.len() > MAX_LABEL_LEN { return Err(EmailError::DomainLabelTooLong); }

    let bytes = label.as_bytes();
    if bytes[0] == b'-'               { return Err(EmailError::DomainLabelLeadingHyphen); }
    if *bytes.last().unwrap() == b'-' { return Err(EmailError::DomainLabelTrailingHyphen); }
...
fn validate_domain(domain: &str) -> Result<(), EmailError> {
    if domain.is_empty()             { return Err(EmailError::DomainEmpty); }
    if domain.as_bytes()[0] == b'['  { return Err(EmailError::DomainIpLiteralForbidden); }
    if domain.len() > MAX_DOMAIN_LEN { return Err(EmailError::DomainTooLong); }
    if domain.ends_with('.')         { return Err(EmailError::DomainTrailingDot); }
    ...
    if body.is_empty() { return Err(EmailError::DomainLabelEmpty); }
    ...
    if tld.len() < MIN_TLD_LEN { return Err(EmailError::DomainTldTooShort); }
    for (i, &b) in tld.as_bytes().iter().enumerate() {
        if b >= 128 {
            let c = tld[i..].chars().next().unwrap();
            return Err(EmailError::NonAscii(c));
        }
        if !b.is_ascii_alphabetic() { return Err(EmailError::DomainTldNotAlpha); }
    }
...
pub fn validate_email(email: &str) -> Result<ValidatedEmail<'_>, EmailError> {
    ...
    if email.len() > MAX_EMAIL_LEN { return Err(EmailError::TooLong); }
    ...
    for (i, &b) in email.as_bytes().iter().enumerate() {
        if b == b'@' {
            if at_pos.is_some() { return Err(EmailError::MultipleAt); }
            at_pos = Some(i);
        }
    }
